...
void SensorInitTask(void *pvParameters) {
    ...
    uint8_t acc_ctrl1 = 0x67;
    i2c_mem_write(ACC_ADDRESS, CTRL_REG1_A, &acc_ctrl1, 1, pdMS_TO_TICKS(100));

    uint8_t acc_ctrl4 = 0x80;
    i2c_mem_write(ACC_ADDRESS, CTRL_REG4_A, &acc_ctrl4, 1, pdMS_TO_TICKS(100));

    uint8_t mag_cra = 0x10;
    i2c_mem_write(MAG_ADDRESS, CRA_REG_M, &mag_cra, 1, pdMS_TO_TICKS(100));

    uint8_t mag_crb = 0x20;
